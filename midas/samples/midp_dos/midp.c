...
//#endif
    ...
    else
    {
        ...
        else
        {
            ...
            {
                puts("Loading Impulse Tracker module");
                if ( realVU )
                {
                    if ( (error = gmpLoadIT(argv[1], 1, &MakeMeter, &module))
                         != OK )
                        midasError(error);
                }
                else
                {
                    if ( (error = gmpLoadIT(argv[1], 1, NULL, &module))
                         != OK )
                        midasError(error);
                }
            }
            else
            {
                ...
                {
                    if ( (error = gmpLoadMOD(argv[1], 1, NULL, &module))
                         != OK )
                        midasError(error);
                }
            }
