...
static void Set80x50(void)
{
    ...
    _farpokeb(_dos_ds, 0x487, _farpeekb(_dos_ds, 0x487) | 1);
    ...
    _farpokeb(_dos_ds, 0x487, _farpeekb(_dos_ds, 0x487) & 0xFE);

    /*outportw(0x3D4, 0x0814);*/
    outportb(0x03D4, 0x14);
    outportb(0x03D5, 0x08);
...
void InitDisplay(void)
{
    ...
    /* Warning! Works only for large model in real mode and _FLAT_ 32-bit
       modes */
#ifdef __DJGPP__
    oldDispMode = _farpeekb(_dos_ds, 0x449);
    oldColumns = _farpeekw(_dos_ds, 0x44A);
    oldRows = _farpeekb(_dos_ds, 0x484) + 1;
#else
...
void DrawInstNames(void)
{
    ...
    if ( (error = memAlloc(256, (void**) &s)) != OK )
        midasError(error);

    /* Allocate memory for channel ID characters: */
    if ( (error = memAlloc(numChannels+1, (void**) &c)) != OK )
        midasError(error);
...
void DrawSongInfo(void)
{
    ...
    if ( (error = midasSD->GetMixRate(&mixRate)) != OK )
        midasError(error);
#ifdef __DJGPP__
    mStrAppend(itoa(mixRate, &s[0], 10), "Hz");
#else
