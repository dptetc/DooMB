...
bool DCajunMaster::Check_LOS (AActor *from, AActor *to, angle_t vangle)
{
	...
	return abs(int(R_PointToAngle2 (from->x, from->y, to->x, to->y) - from->angle)) <= (int)vangle/2;
}
...
void DCajunMaster::Dofire (AActor *actor, ticcmd_t *cmd)
{
	...
	case wp_missile: //Special rules for RL
		...
		if(an)
		{
			...
			if (abs(int(actor->player->angle - actor->angle)) < 12*ANGLE_1) //have to be somewhat precise. to avoid suicide.
			{
	...
	default: //Other weapons, mostly instant hit stuff.
		...
		if (abs(int(actor->player->angle - actor->angle)) < 4*ANGLE_1)
		{
