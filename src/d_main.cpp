...
//	  2. $DOOMWADDIR
//	  3. $HOME/.DooMB
//	  4. The share directory defined at compile time (/usr/local/share/DooMB)
//
//==========================================================================
...
void D_DoomMain (void)
{
	...
	{
		// [RH] Make sure DooMB.wad is always loaded,
		// as it contains magic stuff we need.
		const char *wad;

		wad = BaseFileSearch ("DooMB.wad", NULL);
		if (wad)
			D_AddFile (wad);
		else
			I_FatalError ("Cannot find DooMB.wad");
	...
	// [RH] Add any .wad files in the skins directory
	{
		...
		if (getcwd (curdir, 256))
		{
			...
			if (home)
			{
				stuffstart = sprintf (skindir, "%s%s.DooMB/skins", home,
									  home[strlen(home)-1] == '/' ? "" : "/");
				if (!chdir (skindir))
				{
					skindir[stuffstart++] = '/';
