...
void G_BuildSaveName (char *name, int slot)
{
	...
	if (Args.CheckParm ("-cdrom"))
		sprintf(name, "c:\\DooMBdat\\%s%d.zds", SAVEGAMENAME, slot);
	else
...
void G_BeginRecording (void)
{
	...
	WriteWord (GAMEVER, &demo_p);			// Write DooMB version
...
BOOL G_ProcessIFFDemo (char *mapname)
{
	...
	id = ReadLong (&demo_p);
	if (id != ZDEM_ID) {
		Printf (PRINT_HIGH, "Not a DooMB demo file!\n");
		return true;
	}
	...
	while (demo_p < zdemformend && !bodyHit) {
		...
		switch (id) {
			case ZDHD_ID:
				...
				demover = ReadWord (&demo_p);	// DooMB version demo was created with
				if (ReadWord (&demo_p) > GAMEVER) {		// Minimum DooMB version
					Printf (PRINT_HIGH, "Demo requires a newer version of DooMB!\n");
					return true;
				}
