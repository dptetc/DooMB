...
char *GetConfigPath (void)
{
	...
	if (Args.CheckParm ("-cdrom"))
		return copystring ("c:\\DooMBdat\\DooMB.cfg");

	path = new char[strlen (progdir) + 11];

	strcpy (path, progdir);
	strcat (path, "DooMB.cfg");
	return path;
#else
	return GetUserFile ("DooMB.cfg");
#endif
}
...
char *GetAutoexecPath (void)
{
	...
	if (Args.CheckParm ("-cdrom"))
	{
		return copystring ("c:\\DooMBdat\\autoexec.cfg");
	}
	else
	{
...
void STACK_ARGS M_SaveDefaults (void)
{
	...
	{
		fprintf (f, "// Generated by DooMB " DOTVERSIONSTR " - don't hurt anything\n");
