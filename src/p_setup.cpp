...
// errors _can_ cause firelines.
		...
		dis = 0;
		delta_angle = (abs(int(ptp_angle-li->angle)>>ANGLETOFINESHIFT))*360/8192;

		if (delta_angle != 0)
		{
			dx = (li->v1->x - li->v2->x)>>FRACBITS;
			dy = (li->v1->y - li->v2->y)>>FRACBITS;
			dis = 0; //((int) sqrt(dx*dx + dy*dy))<<FRACBITS;
...
void P_LoadThings (int lump)
{
	...
	// [RH] DooMB now uses Hexen-style maps as its native format.
