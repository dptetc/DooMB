...
void P_TranslateLineDef (line_t *ld, maplinedef_t *mld)
{
	...
	{
		if (special >= DooMBStaticInits && special < DooMBStaticInits + NUM_STATIC_INITS)
		{
			// A DooMB Static_Init special
			ld->special = Static_Init;
			ld->args[0] = tag;
			ld->args[1] = special - DooMBStaticInits;
		}
		else
		{
